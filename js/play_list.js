var loadObj={
    ...
    loadLiveFun:function(json,loadLength){
         var _this=this;
         var href=pathObj.setJsonFun('/static/weicity/play_detail.html','/shopPlay/unconfine/playDetail.form');
         for(var i=0;i<json.data.length;i++){
            var html='<li><a href="'+href+'?Id='+json.data[i].playId+'"><img class="main_pic" src="'+imgAddress+'play/'+json.data[i].playId+'/'+json.data[i].imgaddr+'" onerror="imgError(this)"><i class="collect"></i><figure><figcaption>'+json.data[i].playName+'</figcaption><em class="info">'+json.data[i].content+'</em></figure></a></li>';
...
var searchObj={                     //搜索框对象
    ...
    searchFun:function(){
        ...
        postObj.data=data;
         var options={
            type:'get',
            url:pathObj.setJsonFun('/static/weicity/json/play_list.json','/shopPlay/unconfine/search.form'),
            data:data,
            dataType:"json",
