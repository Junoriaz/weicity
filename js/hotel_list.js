    ...
    loadLiveFun:function(json,loadLength){
     var _this=this;
         for(var i=0;i<json.recommend.length;i++){
            var html='<li><a href="'+json.recommend[i].vrUrl+'?Id='+json.recommend[i].hotelId+'"><img class="main_pic" src="'+imgAddress+'hotel/'+json.recommend[i].hotelId+'/'+json.recommend[i].defaultImgThumbUrl+'" onerror="imgError(this)"><i class="collect"></i><span>¥<strong>'+json.recommend[i].livePrice+'</strong>/起</span><figure><figcaption>'+json.recommend[i].hotelName+'</figcaption><em class="info">'+json.recommend[i].briefInfo+'</em></figure></a></li>';
